...
class Social {
	...
	public function linkedin_connect($redirectUrl=null,$site_url,$ldn_api_key,$ldn_secret_key) {
		...
		if(empty($redirectUrl)) {
			$redirectUrl = $site_url.'ldn_login/ldn';
		}
		...
		if(isset($_GET['error'])) {
			...
			$userData = $client->fetch('/v1/people/~:(id,first-name,last-name,picture-url,public-profile-url,email-address,formatted-name,headline,location,industry,positions,num-connections)');
			if(!empty($userData['emailAddress']) && !empty($userData['id'])) {
				$ldnData['oauth_uid'] = $userData['id'];
				$ldnData['email'] = $userData['emailAddress'];
				$ldnData['firstname'] = $userData['firstName'];
				$ldnData['lastname'] = $userData['lastName'];
				$ldnData['formattedName'] = $userData['formattedName'];
				$ldnData['headline'] = $userData['headline'];
				$ldnData['industry'] = $userData['industry'];
				$ldnData['user_type'] = 'linkedin';
				$ldnData['status'] = 1;
				$ldnData['activation_time'] = date('Y-m-d H:i:s');
				$ldnData['country'] = strtoupper($userData['location']['country']['code']);
				$ldnData['locale'] = $userData['location']['name'];
				$ldnData['numConnections'] = $userData['numConnections'];
				$ldnData['companyName'] = $userData['positions']['values'][0]['company']['name'];
				$ldnData['companyId'] = $userData['positions']['values'][0]['id'];
				$ldnData['isCurrent'] = $userData['positions']['values'][0]['isCurrent'];
				$ldnData['companyLoc'] = $userData['positions']['values'][0]['location']['name'];
				$ldnData['startMonth'] = $userData['positions']['values'][0]['startDate']['month'];
				$ldnData['startYear'] = $userData['positions']['values'][0]['startDate']['year'];
				$ldnData['jobTitle'] = $userData['positions']['values'][0]['title'];
				$ldnData['gender'] = (!empty($userData['gender'])) ? $userData['gender'] : '';
				$ldnData['avatar'] = (!empty($userData['pictureUrl'])) ? $userData['pictureUrl'] : '';
				$ldnData['profile_url'] = (!empty($userData['publicProfileUrl'])) ? $userData['publicProfileUrl'] : '';
				$ldnData['activation_token'] = $access_token['access_token'];
			}
		} else {
