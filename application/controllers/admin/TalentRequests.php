...
class TalentRequests extends CI_Controller 
{
	...
	public function submit_talent_request()
	{
		if($this->input->server('REQUEST_METHOD') == 'POST')
		{
			...
			$thursday_counter=0;
			$friday_counter=0;
			// echo "<pre>";print_r(array_values($monday));die();
			...
			if ( ($this->form_validation->run() !== false) && ( $all_counter < 1 && $monday_counter < 1 && $tuesday_counter < 1 && $wednesday_counter < 1 && $thursday_counter < 1 &&  $friday_counter < 1 ) )
			{
	...
	public function clientBusinessRequest()
	{
		...
		$clientOrBussinessArea = $this->General_model->get('ts_users', $objArr=false, $where);
